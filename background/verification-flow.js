(function attachBackgroundVerificationFlow(root, factory) {
  ...
  function createVerificationFlowHelpers(deps = {}) {
    ...
    function isIcloudListMail(mail) {
      const configuredProvider = String(ICLOUD_LIST_PROVIDER || '').trim();
      const provider = String(mail?.provider || '').trim();
      const source = String(mail?.source || '').trim();
      return Boolean(configuredProvider && (provider === configuredProvider || source === configuredProvider))
      ...
      for (let round = 1; round <= totalRounds; round++) {
        ...
        while (true) {
          ...
          try {
            ...
            const result = isIcloudListMail(mail)
              ? await pollIcloudListVerificationCode(step, state, mail, timeoutWindow.payload)
              : await sendToMailContentScriptResilient(
                mail,
                {
                  type: 'POLL_EMAIL',
    ...
    async function recoverVerificationSubmitOutcomeAfterTransportError(step, error, options = {}) {
      if (!isRetryableVerificationTransportError(error)) {
        throw error;
      }
      ...
      if (step === 8) {
        const fallback = await detectStep8PostSubmitFallback({
          step,
          timeoutMs: Math.min(fallbackTimeoutMs, 9000),
          pollIntervalMs: 300,
        });
        ...
        if (fallback.success) {
          await addLog(
            fallback.addPhonePage
              ? '验证码提交后通信中断，但页面已进入手机号验证页，按提交成功继续。'
              : '验证码提交后通信中断，但页面已进入 OAuth 授权页，按提交成功继续。',
            'warn',
            { step: getCompletionStep(step, options), stepKey: 'fetch-login-code' }
          );
          return {
            success: true,
