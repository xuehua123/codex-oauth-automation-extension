...
function getStep4PostVerificationState(options = {}) {
  ...
  if (
    !ignoreVerificationVisibility
    && typeof isVerificationPageStillVisible === 'function'
  ...
  if (
    (typeof isStep5Ready === 'function' && isStep5Ready())
    || (typeof isSignupProfilePageUrl === 'function' && isSignupProfilePageUrl())
  ) {
    return {
      state: 'step5',
      url: location.href,
...
function getVerificationSubmitButtonForTarget(codeInput, options = {}) {
  ...
  const scoreSubmitAction = (element, root) => {
    ...
    const continueActionPattern = typeof CONTINUE_ACTION_PATTERN !== 'undefined'
      ? CONTINUE_ACTION_PATTERN
      : /继续|continue/i;
    ...
    if (continueActionPattern.test(text)) {
      score += 100;
    }
...
async function waitForVerificationAutoAdvanceBeforeClick(step, timeout = 1200) {
  if (typeof getVerificationSubmitState !== 'function') {
    return null;
  }

  const start = Date.now();
  const waitingState = step === 4 ? 'verification' : 'verification_page';
  const pendingStates = new Set([waitingState, 'unknown', 'idle', '']);
  ...
  while (Date.now() - start < timeout) {
    ...
    if (snapshot?.state && !pendingStates.has(snapshot.state)) {
      return snapshot;
    }
...
async function fillVerificationCode(step, payload) {
  ...
  if (step === 4) {
    if (typeof waitForDocumentLoadComplete === 'function') {
      await waitForDocumentLoadComplete(15000, `步骤 ${step}：注册验证码页面`);
    }
  }

  const combinedSignupProfilePage = step === 4
    && typeof waitForCombinedSignupVerificationProfilePage === 'function'
  ...
  if (splitInputs?.length >= 6) {
    ...
    await sleep(500);
    const splitAutoAdvanceState = typeof waitForVerificationAutoAdvanceBeforeClick === 'function'
      ? await waitForVerificationAutoAdvanceBeforeClick(step, 1200)
      : null;
  ...
  // Submit
  await sleep(500);
  const autoAdvanceState = typeof waitForVerificationAutoAdvanceBeforeClick === 'function'
    ? await waitForVerificationAutoAdvanceBeforeClick(step, 1200)
    : null;
  if (!autoAdvanceState) {
